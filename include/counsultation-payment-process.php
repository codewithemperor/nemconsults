...
if(isset($_GET['status'])) {
    $gatewayType = $_GET['gateway'] ?? 'flutterwave'; // Default to Flutterwave
    ...
    elseif($_GET['status'] == 'successful') {
        // Determine gateway and verification parameters
        if($gatewayType === 'paystack') {
            ...
            $verify_url = "https://api.flutterwave.com/v3/transactions/{$txid}/verify";
        }
        ...
        // Process verification based on gateway
        if($gatewayType === 'paystack') {
        ...
        if($isVerified) {
            ...
            if ($stmt->execute()) {
                // Email sending function (same for both gateways)
                try {
            ...
            $errorMessage = $gatewayType === 'paystack' 
                ? ($res->message ?? 'Payment verification failed') 
                : ($res->message ?? 'Unable to verify payment');
