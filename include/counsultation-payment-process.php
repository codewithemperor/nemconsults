...
if (isset($_GET['status'])) {
    ...
    if ($_GET['status'] == 'cancelled') {
        ...
        if ($res->status === 'success') {
            ...
            if ($amountPaid >= $amountToPay) {
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssssssssssssss", $package, $surname, $otherName, $phoneNumber, $email, $appointmentDate, $address, $visaValidity, $visaRefusal, $visaType, $details, $aboutUs, $amountPaid, $amountToPay, $paymentStatus, 'flutterwave');

                if ($stmt->execute()) {
                    // Send emails (same as your existing logic)
                    sendEmails($package, $surname, $otherName, $phoneNumber, $email, $appointmentDate, $address, $visaValidity, $visaRefusal, $visaType, $details, $aboutUs, $amountPaid, $paymentStatus);
                ...
                die('Fraudulent transaction: Amount paid does not match expected amount.');
            }
        } else {
            die('Payment verification failed: ' . ($res->message ?? 'Unknown error'));
        }
    }
} elseif (isset($_GET['trxref']) || isset($_GET['reference'])) {
    // Handle Paystack response
    $reference = $_GET['trxref'] ?? $_GET['reference'];
    ...
    if ($res->status === true && $res->data->status === 'success') {
        // Extract payment details
        $amountPaid = $res->data->amount / 100; // Convert from kobo/cents
        $metaData = $res->data->metadata;
        $amountToPay = $metaData->packageAmount;
        ...
        if ($amountPaid >= $amountToPay) {
            ...
            $visaValidity = $metaData->visaValidity ?? '';
            $visaRefusal = $metaData->visaRefusal;
    ...
    die('Invalid payment gateway response.');
}

// Function to send emails (same as your existing logic)
function sendEmails($package, $surname, $otherName, $phoneNumber, $email, $appointmentDate, $address, $visaValidity, $visaRefusal, $visaType, $details, $aboutUs, $amountPaid, $paymentStatus) {
    try {
        ...
        $mail->addAddress('info@nemconsults.com', 'Nemconsults'); // Admin email
